...
void main_ellip3d(float disp_top, float disp_bot, int num_runs, int num_threads, std::string dirName, double dt)
{
    ...
    // suggestion: the inputfile and outputfile are defined in the function before calling 
    // numberOfCalls starting from 1
...
//    A.unixialCompression(50,	// the timestep used in FEM and DEM may be different, so control the total_steps to keep the time the same
...
//	    10,  		// print interval for progress file
//	    "input_particle_file", 
//	    "input_boundary_file",
